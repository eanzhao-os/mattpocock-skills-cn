...
当接口本身的结构存在争议时 —— 例如模块应该有多深、接缝到底该设在哪里、接口应该暴露哪些能力 —— 可以通过 Skill 工具调用 "codebase-design" 获取这套词汇。它是模块（module）、接口（interface）、深度（depth）、接缝（seam）、适配器（adapter）、杠杆率（leverage）和局部性（locality）等术语的共享权威定义来源；它是一份供随时查阅的参考，而不是需要独立运行的会话。
