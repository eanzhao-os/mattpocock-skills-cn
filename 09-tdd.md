...
测试必须通过**对外公共接口（public interfaces）**来验证行为，而不是紧贴内部实现细节。内部代码可以被完全重构改写，但测试不应该随之挂掉。一个好的测试读起来应该像一份活的需求规范 —— 例如“在购物车有效时用户可以正常结账（user can checkout with valid cart）”能够精确传达系统具备何种能力 —— 并且在后续的代码重构中依然稳健存活，因为它的断言根本不关心内部的具体代码结构。

关于好坏测试的对比示例请参阅 [tests.md](./09-tdd_tests.md)，关于 Mock 模拟的边界指南请参阅 [mocking.md](./09-tdd_mocking.md)。

## 架构接缝（Seams）—— 测试应当写在哪里

**接缝（Seam）** 是你在其上进行测试的公共对外边界：也就是你能观察到系统外部行为而无需探入其内部实现细节的接口。测试必须编写在接缝上，绝不要面向内部私有实现。

**只在预先确认的接缝上编写测试**。在编写任何测试之前，必须先明确写出待测的接缝，并向用户确认。**绝不在未与用户确认的接缝上编写测试**。你不可能测试所有细节 —— 提前敲定接缝，才能把测试的精力聚焦在核心关键路径和复杂业务逻辑上，而不是浪费在每个细枝末节的极端情况上。
...
- **与实现细节强耦合（Implementation-coupled）**：过度 Mock 内部协作对象、测试私有方法、或者通过旁门左道进行验证（例如不走接口而直接去查底层数据库）。典型特征：当你仅仅做了一次内部重构、外部行为完全没变时，测试却大面积报错挂掉。
- **同义反复 / 虚假自证（Tautological）**：断言计算预期值的方式与被测代码完全一样（例如 `expect(add(a, b)).toBe(a + b)`、手推得到的快照 snapshot、或者断言某个常量等于它自己）。这种测试在结构上永远都是绿灯通过的，永远无法与代码产生真正的分歧。测试的期望值必须来自一个**独立的权威真相源** —— 例如已知正确的常量字面量、手工验证过的具体案例、或需求规范文档。
- **水平横切（Horizontal slicing）**：一次性把所有测试全部写完，再去写所有实现代码。批量先写测试验证的只是**脑海中脑补想象的**行为：你测试的只是事物的外部表象轮廓（shape），而非用户真实的端到端行为；这会导致测试对真正的变化迟钝不敏感，并且在真正理解实现前就锁死了僵化的测试结构。必须改为 **垂直切片（vertical slices）** 推进 —— 写一个测试 → 写一段最小实现使其变绿 → 重复此过程；每一个测试都是发**穿透示踪弹（tracer bullet）**，紧密响应上一个循环中学到的新认知。
