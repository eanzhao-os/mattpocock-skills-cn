# 15-domain-modeling / ADR-FORMAT.md 精读（架构决策记录格式与准入门槛规范（ADR Format））

所有的架构决策记录（ADR）均保存在 `docs/adr/` 目录下，并使用四位自增数字编号命名：`0001-slug.md`、`0002-slug.md` 等。新建 ADR 时，扫描 `docs/adr/` 找出现有的最大编号，然后加一即可。

**按需惰性创建（Lazy）** `docs/adr/` 目录 —— 仅在真正需要落盘第一篇 ADR 时才创建。
...
- **状态属性（Status frontmatter）**：`proposed`（提议中） / `accepted`（已采纳） / `deprecated`（已废弃） / `superseded by ADR-NNNN`（已被某号 ADR 替代） —— 在决策被推翻重议时非常有用；
...
2. **缺乏背景时令人困惑（Surprising without context）** —— 未来的代码维护者在看到这段实现时，会忍不住惊讶纳闷：“他们当初到底为什么非要这么干？”；
3. **真实权衡取舍的结果（The result of a real trade-off）** —— 当初确实存在切实可行的多种备选路径，而我们因为非常具体的理由选择了其中一种。
...
- **清晰的边界划定与职责切割**：“客户核心数据严格归客户上下文独占所有，其他上下文只允许通过 ID 间接弱引用”。明确拒绝做什么与声明做什么同样有价值；
- **刻意偏离常规道路的反直觉抉择**：“出于 X 的原因，我们手写原生 SQL 而非使用 ORM”。任何一个明理的工程师第一反应都会选择相反方案的地方，必须立 ADR 存照 —— **正是这篇文档，能够阻止下一个自以为聪明的工程师把这处深思熟虑的精妙设计当成‘缺陷’给盲目‘修复’掉**；
- **代码本身完全不可见的外部硬性约束**：“出于严格的合规审计要求，本项目严禁接入 AWS”、“出于与外部伙伴签署的合同 SLA，API 响应时间必须控制在 200ms 以内”；
