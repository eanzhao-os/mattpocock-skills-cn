# 27. loop-me（自动化工作流深度访谈与规格化）

```yaml
name: loop-me
description: 在当前工作区内，围绕我希望构建的工作流自动化规范（workflow specs），对我进行一场刨根问底式的深度访谈。
disable-model-invocation: true
argument-hint: "指定一个希望设计的工作流，或者留空以便共同挖掘探索"
```

运行一个有状态的 `/grilling` 深度访谈会话，其**唯一的最终产出**是 **工作流规范文件（workflow specs）**。严格遵循深度访谈纪律 —— 穷追不舍、一次只提一轮问题、且为每一个问题都附带一个推荐方案 —— 紧紧围绕下方的核心词汇与终极目标展开。随着访谈逐步敲定各项决策，在工作区中动态创建、编辑或删除对应的工作流规范文件。

---

## 循环视角（The loop lens）

所谓 **循环（loop）**，是指用户生活中反复出现的周期性模式：职业日常、每周例程、早晨习惯、或是某项反复进行的操作。将现实生活具象化为“大循环套着小循环”，能够清晰暴露日常活动中极高的可预测性 —— 而这种高度可预测的特征，正是让它们**极具自动化委派价值（worth delegating）**的根本原因。使用这一视角去发现那些值得被转化为规范文件的循环，并主动向用户提议那些他们自己尚未意识到的潜在循环。
...
这是一套共享的语言体系，**仅在某个具体工作流确实需要时才调阅** —— 绝不要当成死板的打勾检查清单。**不要预设任何僵化的固定结构**：除非深度访谈表明确实需要，否则一个工作流完全可以不需要 AI 介入、不需要任何人工检查点、也不需要任何定时调度。
...
一份工作流规范文件宣告彻底完成的标准是：**负责具体实现的 Agent 在阅读该文档后，无需再向用户提哪怕一个疑问，就能独立将其完整构建出来。** 在此之前持续追问；只要还有任何一个悬而未决的疑问，该规范就算不上完成。
...
- `NOTES.md` — 记录用户现实世界原始信息的基础备忘录：用户所使用的工具链、他们日常处理的沟通渠道、以及他们对两者使用的个性化术语。如果该文件内容为空或过于单薄，在着手编写任何工作流规范之前，先围绕用户的日常世界进行深度访谈。随着模糊术语的浮现，当场将其打磨为规范词汇并记录在此处。
...
| 触发 | 在本工作区内，深度访谈用户以产出 workflow specs |
