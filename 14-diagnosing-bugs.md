...
description: 针对疑难杂症与性能回归的系统化诊断循环。当用户表示需要 "diagnose/debug this（诊断/调试此问题）"，或者反馈系统出现损坏、报错抛出异常、运行失败或卡顿变慢时触发。
```
...
所谓“正确的接缝”，是指测试能够完整演练该 Bug 在实际调用点处发生的**真实错误模式**。如果当前唯一的可用接缝过浅（例如单调用方的单元测试根本无法还原触发 Bug 的整条链条），在此处编写回归测试只会带来虚假的安全感。
...
**最后必须反思：怎样做才能在最初就彻底预防此类 Bug 的发生？** 如果答案涉及架构层面的改进（例如缺乏良好的测试接缝、调用关系错综复杂、存在隐蔽的耦合），请将详细细节无缝交接给 `/improve-codebase-architecture` skill。**请务必在修复代码完全落地之后再提出此类架构建议**，而不要在刚开始时提 —— 因为此时你掌握的信息远比刚开始排查时要丰富和透彻得多。
