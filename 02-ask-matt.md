...
```yaml
name: ask-matt
description: 咨询在当前场景下适合使用哪个 skill 或工作流。本仓库所有 skills 的总路由指南。
disable-model-invocation: true
```

# Ask Matt（咨询 Matt：工作流导航）
...
## 主流程：从想法到交付（The main flow: idea → ship）
...
**`/setup-matt-pocock-skills`** — 在开始第一次工程工作流之前运行，用于配置其他 skills 默认依赖的工单跟踪系统、分类标签以及文档结构布局。也支持自定义的工单跟踪系统。
