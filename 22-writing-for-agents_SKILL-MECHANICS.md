# 22-writing-for-agents / SKILL-MECHANICS.md 精读
...
# Skill 底层机制与路由模式（Skill Mechanics）
...
- **人类可达性**：人类用户依然可以随时手动输入斜杠命令触发 —— **模型调用永远超集包含人类手动调用**；赋予描述字段仅仅是为 Agent 增加了自动发现能力，绝不会削弱人类的调用权；
- **核心代价**：`description` 字段作为最顶层的上下文指针，**会被强制常驻加载在系统的系统提示词中** —— 这相当于付出了永续常驻的 Token 上下文负载（Context load），换取了随时随地的被发现能力；
- **代码实现**：不设置 `disable-model-invocation`，并在 `description` 中写满面向模型理解的分流引导词分支。
