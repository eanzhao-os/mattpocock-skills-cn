...
- **定义必须极其紧凑（Keep definitions tight）**：最多一至两句话。**清晰定义它“是什么（what it IS）”，而不是去长篇大论它“怎么做（what it does）”**；
...
- 如果两者均不存在，在首次敲定并澄清第一个领域术语时，**按需惰性创建（Lazy）**根 `CONTEXT.md`；
- 在多上下文共存时，智能推断当前任务究竟从属于哪个具体上下文；若存在歧义，主动向用户询问确认。
